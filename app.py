...
st.markdown(
    ...
    * {
    ...
    div.nav-container label:hover {
        background-color: #020617;
        border-color: #1f2937;
        color: #e5e7eb;
    }

    /* estado selecionado do radio (aba ativa) */
    div.nav-container input[role="radio"]:checked + div {
...
# ============================================================
# 4. DESCOBRIR BLOCOS DE DATAS (GENÉRICO)
# ============================================================

def descobrir_blocos_datas(df: pd.DataFrame):
    """
    Identifica todos os blocos de ausência com base na estrutura:

    Início / Inicio  → Fim / FIm → [D] → [Motivo ou Curso opcional]

    Regra:
      - Se não houver coluna 'Motivo' nem 'Curso' à direita ⇒ Tipo_base = "Férias"
      - Se houver 'Motivo' ⇒ Tipo_base = "Outros"
      - Se houver 'Curso' ⇒ Tipo_base = "Curso"
    ...
    for i, nome_col in enumerate(cols):
        ...
        # 2) Encontrar Motivo ou Curso mais próximo à direita
        k = None
        tipo_base = "Férias"
        for idx3 in range(j + 1, len(cols)):
            n3 = str(cols[idx3])
            if "Motivo" in n3:
                k = idx3
                tipo_base = "Outros"
...
# ============================================================
# 11. LAYOUT PRINCIPAL: NAV LATERAL + CONTEÚDO
# ============================================================
